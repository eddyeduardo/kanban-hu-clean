  ...
  if (propStories.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow p-6 text-center">
        ...
        <h3 className="text-lg font-medium text-slate-700 mb-2">No hay historias disponibles</h3>
        <p className="text-slate-500">Agrega historias para ver el plan de pruebas</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow overflow-hidden">
      <div className="p-4 border-b border-slate-200 flex justify-between items-center">
        <h2 className="text-lg font-semibold text-slate-800">Plan de Pruebas</h2>
        <div className="flex space-x-2">
          ...
          >
            <FiFileTextIcon className="mr-2" />
            <span>Excel</span>
          </button>
          <button
            onClick={exportToCSV}
            disabled={exporting}
            className="px-3 py-1.5 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 flex items-center"
          ...
          <button
            ...
            className={`px-3 py-1.5 ${propStories.length === 0 ? 'bg-gray-400' : 'bg-red-600 hover:bg-red-700'} text-white text-sm font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 flex items-center`}
          ...
          <button
            ...
            className="px-3 py-1.5 bg-slate-600 text-white text-sm font-medium rounded-md hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 flex items-center"
            title="Imprimir"
          >
          ...
          <tbody className="bg-white divide-y divide-slate-200">
            {sortedColumns.flatMap(column => {
              ...
              return columnStories.flatMap((story, storyIndex) => {
                if (!story.criteria || story.criteria.length === 0) {
                  return (
                    <tr key={`${story._id || storyIndex}-no-criteria`} className="bg-slate-50">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-500 italic" colSpan="2">
                ...
                return story.criteria.map((criterion, critIndex) => (
                  <tr 
                    ...
                    className={critIndex % 2 === 0 ? 'bg-white' : 'bg-slate-50'}
                  >
                    {critIndex === 0 && (
                      <>
                        <td 
                        ...
                        <td 
                          className="px-6 py-4 whitespace-nowrap text-sm text-slate-500 border-r border-slate-200"
                          rowSpan={story.criteria.length}
                        >
                          {story.id_historia || 'N/A'}
                        </td>
                        <td 
                          className="px-6 py-4 whitespace-normal text-sm text-slate-900 border-r border-slate-200"
                          rowSpan={story.criteria.length}
                        >
                          {story.title || 'Sin título'}
                        </td>
                    ...
                    <td className="px-6 py-4 text-sm text-slate-900 whitespace-normal">
                      {criterion.text || 'Sin descripción'}
                    </td>
      ...
      {exporting && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 shadow-xl max-w-md w-full mx-4">
            ...
            <p className="text-center text-gray-700">Generando archivo, por favor espere...</p>
          </div>
        </div>
      )}
    </div>
